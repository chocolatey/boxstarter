...
task Create-ModuleZipForRemoting {
    ...
    ."$env:chocolateyInstall\bin\7za" a -tzip "$basedir\buildartifacts\Boxstarter.zip" "$basedir\boxstarter.Common" | out-Null
    ."$env:chocolateyInstall\bin\7za" a -tzip "$basedir\buildartifacts\Boxstarter.zip" "$basedir\boxstarter.WinConfig" | out-Null
    ."$env:chocolateyInstall\bin\7za" a -tzip "$basedir\buildartifacts\Boxstarter.zip" "$basedir\boxstarter.bootstrapper" | out-Null
    ."$env:chocolateyInstall\bin\7za" a -tzip "$basedir\buildartifacts\Boxstarter.zip" "$basedir\boxstarter.chocolatey" | out-Null
    ."$env:chocolateyInstall\bin\7za" a -tzip "$basedir\buildartifacts\Boxstarter.zip" "$basedir\boxstarter.config" | out-Null
    ."$env:chocolateyInstall\bin\7za" a -tzip "$basedir\buildartifacts\Boxstarter.zip" "$basedir\license.txt" | out-Null
...
Task Package-DownloadZip -depends Clean-Artifacts {
    ...
    exec { ."$env:chocolateyInstall\bin\7za" a -tzip "$basedir\BuildArtifacts\Boxstarter.$version.zip" "$basedir\license.txt" }
    exec { ."$env:chocolateyInstall\bin\7za" a -tzip "$basedir\BuildArtifacts\Boxstarter.$version.zip" "$basedir\buildscripts\bootstrapper.ps1" }
    exec { ."$env:chocolateyInstall\bin\7za" a -tzip "$basedir\BuildArtifacts\Boxstarter.$version.zip" "$basedir\Setup.bat" }
}
